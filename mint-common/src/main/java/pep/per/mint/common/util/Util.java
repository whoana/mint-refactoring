...
//			if(logger.isDebugEnabled()) logger.debug(Util.join("Exception is occurred When getting ListenerService's property[",key,"], so return defaultValue[",defaultValue, "]", e));
	...
	public static String getDateAddFromToday(String pattern, int addDate) {
		DateFormat dateFormat = new SimpleDateFormat(pattern);
		//Date today = null;
//
//		try {
//			//today = dateFormat.parse(new Date().toString());
//		} catch (ParseException e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
//		}
	...
	/**
	 * 출처: http://javafactory.tistory.com/1284 [FreeLife의 저장소]
	 * @param str
	 * @return
	 */
	...
	public static String encodeBase64URLData(String data) throws Exception{
		return encodeBase64URLData(data, "utf-8");
	}

	public static String encodeBase64URLData(String data, String charset) throws Exception{
		return new BASE64Encoder().encode(URLEncoder.encode(data, charset).getBytes(charset));
	}


	public static String decodeBase64URLData(String data, String charset)throws Exception{
		return URLDecoder.decode(new String(new BASE64Decoder().decodeBuffer(data), charset), charset) ;
	}

	public static String decodeBase64URLData(String data)throws Exception{
		return decodeBase64URLData(data, "utf-8") ;
	}
