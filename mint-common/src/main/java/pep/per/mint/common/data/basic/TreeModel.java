...
public class TreeModel<T> extends CacheableObject{

    @JsonProperty
    List<ItemModel<T>> roots;

    @JsonProperty
    String text = "ROOT";

    public void addRoot(ItemModel<T> root){
        if(roots == null) {
            roots = new ArrayList<ItemModel<T>>();
        }
        roots.add(root);
    }

    public List<ItemModel<T>> getRoots() {
        return roots;
    }

    public void setRoots(List<ItemModel<T>> roots) {
        this.roots = roots;
    }
