...
import pep.per.mint.database.service.ut.SHLSiteDataUploadService;
...
@Controller
@RequestMapping("/ut")
public class SHLSiteDataUploadController {

	/**
	 * The Logger.
     */
	Logger logger = LoggerFactory.getLogger(SHLSiteDataUploadController.class);
	...
	SHLSiteDataUploadService sHLSiteDataUploadService;
     ...
     */
	@RequestMapping(
    		value="/excel/upload/site/shl/basedata",
	    	...
	    	try {
	            if (!multipartFile.isEmpty()) {
	            	...
	            	if (!filePath.isDirectory() || !filePath.exists()) {
	 				...
	 				//----------------------------------------------------------------------------
	 				//20201211 신한생명 소프트캠프 서버 DRM 해제 처리
	 				//----------------------------------------------------------------------------
	 				if(env.drmApiUseYn.equalsIgnoreCase("Y")) {
	 					String decFilePath = sHLSiteDataUploadService.decDrm(envFilePath, filePostFix);
		 				excelFile = new File( decFilePath );	 					
	 				}	 				
					...
					//----------------------------------------------------------------------------
	 				...
	 				if( multipartFile.getContentType().equals(EXCEL_TYPE_XLSX)
	 					...
	 					if( !Util.isEmpty( item ) ) {
	 						switch( item ) {
	 						case EXECUTE_ALL :
	 							sHLSiteDataUploadService.executeAll(excelData, params);
	 							break;
	 						case EXECUTE_INT :
	 							logger.debug("executeInterface check : "+excelData.toArray()+" // "+deleteYn+" // "+params);
	 							sHLSiteDataUploadService.executeInterface(excelData, deleteYn, params);
	 							break;
	 						case EXECUTE_BSE :
	 							sHLSiteDataUploadService.executeBaseData(excelData, true, params);
	 							break;
	 						}
	 					} else {
	 						//----------------------------------------------------------------------------
