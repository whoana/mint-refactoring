...
import pep.per.mint.database.service.su.EnvironmentService;
 ...
 * <blockquote>
 * <pre>
 * <B> 지원(포탈환경설정) CRUD  서비스 제공 RESTful Controller</B>
 * -------------------------------------------------------------
 * 개발할 메소드 목록
 * -------------------------------------------------------------
...
public class EnvironmentController {
	...
	/**
	 * The Environment Attribute service.
	 */
	@Autowired
	...
	public @ResponseBody ComMessage<Map, List<Environment>> getEnvironmentList(
			HttpSession  httpSession,
			@RequestBody ComMessage<Map, List<Environment>> comMessage,
		...
		//----------------------------------------------------------------------------
		{
			...
			//조회
			//-----------------------------------------------
			list = environmentService.getEnvironmentList(params);
     ...
     * @throws Exception the exception
     */
	@RequestMapping(value = "/environments", params = "method=POST", method = RequestMethod.POST, headers = "content-type=application/json")
	public @ResponseBody ComMessage<Environment, Environment> createEnvironment(
			HttpSession httpSession,
			@RequestBody ComMessage<Environment, Environment> comMessage, Locale locale, HttpServletRequest request)
			throws Exception, ControllerException {
		//----------------------------------------------------------------------------
		// 여긴 비지니스 코드만 작성해라.
		//----------------------------------------------------------------------------
		{
			...
			Environment environment = comMessage.getRequestObject();
			//----------------------------------------------------------------------------
			// 필수필드 체크하기
			//----------------------------------------------------------------------------
			{
			...
			//----------------------------------------------------------------------------
			{
				...
				environment.setRegId(regId);
				environment.setRegDate(regDate);
			}
			...
			try{
				...
				int res = 0;
				{
					res = environmentService.createEnvironment(environment);
					//frontManagementService
				...
				//--------------------------------------------------
				{
					...
					errorCd = MessageSourceUtil.getErrorCd(messageSource, "success.cd.ok", locale);
					errorMsg = MessageSourceUtil.getErrorMsg(messageSource, "success.msg.ok", null, locale);
					comMessage.setResponseObject(environment);
	 ...
	 * @throws Exception the exception
	 */
	@RequestMapping(value = "/environments", params = "method=PUT", method = RequestMethod.POST, headers = "content-type=application/json")
	public @ResponseBody ComMessage<Environment, Environment> updateEnvironment(
			HttpSession httpSession,
			@RequestBody ComMessage<Environment, Environment> comMessage, Locale locale, HttpServletRequest request)
			throws Exception, ControllerException {
		//----------------------------------------------------------------------------
		...
		//----------------------------------------------------------------------------
		{
			...
			Environment application = comMessage.getRequestObject();
			//----------------------------------------------------------------------------
			// 필수필드 체크하기
			//----------------------------------------------------------------------------
			{
			...
			try{
				...
				//----------------------------------------------------------------------------
				int res = 0;
				environmentService.updateEnvironment(application);
				frontManagementService.init();
				//--------------------------------------------------
				// 서비스 처리 종료시간을 얻어 CM에 세팅한다.
				//--------------------------------------------------
	 ...
	 * @throws Exception the exception
	 */
	@RequestMapping(value = "/environments", params = "method=DELETE", method = RequestMethod.POST, headers = "content-type=application/json")
	public @ResponseBody ComMessage<Environment, Environment> deleteEnvironment(
			HttpSession httpSession,
			@RequestBody ComMessage<Environment, Environment> comMessage, Locale locale, HttpServletRequest request)
			throws Exception, ControllerException {
		//----------------------------------------------------------------------------
		...
		//----------------------------------------------------------------------------
		{
			...
			try{
				...
				int res = 0;
				{
					...
					res = environmentService.deleteEnvironment(environment);
					frontManagementService.init();
				}
				//--------------------------------------------------
				// 서비스 처리 종료시간을 얻어 CM에 세팅한다.
