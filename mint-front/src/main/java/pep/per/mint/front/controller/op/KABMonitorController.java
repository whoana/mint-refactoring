...
import pep.per.mint.database.service.op.KABMonitorService;
...
@Controller
@RequestMapping("/op")
public class KABMonitorController {
	private static final Logger logger = LoggerFactory.getLogger(KABMonitorController.class);
	...
	@Autowired
	KABMonitorService monitorService;
	...
	@RequestMapping(value = "/kab/message/logs", params = "method=GET", method = RequestMethod.POST, headers = "content-type=application/json")
	public @ResponseBody ComMessage<Map<String,Object>, List<MessageLog>> getMessageLogList(
			HttpSession httpSession,
			@RequestBody ComMessage<Map<String,Object>, List<MessageLog>> comMessage,
		...
		// ----------------------------------------------------------------------------
		{
			...
			List<MessageLog> list = null;
			{
				...
				if( totalCount == 0 ) {
					totalCount = monitorService.getMessageLogListTotalCount(params);
				}

				if(totalCount > 0) {
					list = monitorService.getMessageLogList(params);
				} else {
					list = new ArrayList();
				}
	 ...
	 * @throws Exception the exception
	 * @author Solution TA
	 * @since version 3.0(2019.12)
	 */
	@RequestMapping(
			value="/kab/message/logs/{logKey1}/{logKey2}/{isDecrypt}", params={"method=GET"}, method=RequestMethod.POST, headers="content-type=application/json")
	public @ResponseBody ComMessage<Map<String, Object>, MessageLog> getMessageLogDetail(
			HttpSession  httpSession,
			@PathVariable("logKey1") String logKey1,
			@PathVariable("logKey2") String logKey2,
			@PathVariable("isDecrypt") String isDecrypt,
			@RequestBody ComMessage<Map<String, Object>, MessageLog> comMessage,
			Locale locale,
		...
		//----------------------------------------------------------------------------
		{
			...
			comMessage.getRequestObject().put("isDecrypt", isDecrypt);

			MessageLog messageLog = monitorService.getMessageLogDetail(comMessage.getRequestObject());
			...
			String errorCd = "";
			String errorMsg = "";
			if (messageLog == null) {// 결과가 없을 경우 비지니스 예외 처리
				errorCd = MessageSourceUtil.getErrorCd(messageSource, "error.cd.retrieve.none", locale);
				errorMsg = MessageSourceUtil.getErrorMsg(messageSource, "error.msg.retrieve.none", null, locale);
			} else {// 성공 처리결과
				errorCd = MessageSourceUtil.getErrorCd(messageSource, "success.cd.ok", locale);
				...
				errorMsg = MessageSourceUtil.getErrorMsg(messageSource, "success.msg.retrieve.list.ok", errorMsgParams, locale);
				comMessage.setResponseObject(messageLog);
			}
			comMessage.setErrorCd(errorCd);
			comMessage.setErrorMsg(errorMsg);
     ...
     */
	@RequestMapping(
			value="/kab/tracking/logs/{logKey1}/{logKey2}/{isDecrypt}", params={"method=GET"}, method=RequestMethod.POST, headers="content-type=application/json")
