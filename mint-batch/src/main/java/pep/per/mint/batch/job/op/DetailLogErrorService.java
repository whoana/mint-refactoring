...
@Service
public class DetailLogErrorService extends JobServiceCaller{

    @Autowired
    DetailLogErrorJobMapper detailLogErrorJobMapper;

    @Transactional
    public void run(Map params) throws Exception{
    	List<Map> list = detailLogErrorJobMapper.selectDelayedTrLog(params);
    	for (Map map : list) {
    		detailLogErrorJobMapper.insertDetailLogError(map);
    		detailLogErrorJobMapper.updateMasterlogStatus(map);
    	}
    }
