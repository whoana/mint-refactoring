...
@Service
public class InterfaceLogErrorKHNMCJob implements Job{

    Logger log = LoggerFactory.getLogger(InterfaceLogErrorKHNMCJob.class);
    ...
    public void execute(JobExecutionContext context)
        ...
        try {
        	...
        	if(sendSMSService.checkSendSMS(new HashMap())) {
        		...
        		List<Map> list = sqlSession.selectList("pep.per.mint.batch.mapper.op.InterfaceLogErrorKHNMCJobMapper.selectErrorTrLog", params);
        		for (Map map : list) {
...
//    				contents = sb.toString();
        			...
        			List<Map> listUser = sqlSession.selectList("pep.per.mint.batch.mapper.op.InterfaceLogErrorKHNMCJobMapper.selectSendUser", intfId);
        			...
        			for(Map userMap : listUser){
        				if(userMap != null  && userMap.get("hpNum") != null && ((String)userMap.get("hpNum")).trim().length()>0){
        					...
        					map.put("subject", userMap.get("userNm")); // 담당자
        					sqlSession.insert("pep.per.mint.batch.mapper.op.InterfaceLogErrorKHNMCJobMapper.insertInterfaceErrLogSms", map);
        				}
        			}
        		}
