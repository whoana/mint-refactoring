...
public class InHouseExcelUploadService implements IExcelUploadService {
							...
							case 3 :
								...
								if( isSeqGroup ) break;
								{
									...
									if( Util.isEmpty(cols[j]) ) {
										...
										// 임시테이블저장시에는 해당 컬럼의 최대사이즈(255) 이상이 되면 안되기 때문에 자른다.
										cutString = strCut(cols[j].trim(),null,100,0,true,true);
										cols[j] = cutString;
									}
								}
								break;
							...
							case 38 :
								...
								if( isSeqGroup ) break;
								{
									if( Util.isEmpty(cols[j]) ) {
										...
										String realFinYear = cols[j].substring(0, 4);
										String realFinMonth = cols[j].substring(5, 7);
										String realFinDay = cols[j].substring(8);
										String realFinYmd = realFinYear + realFinMonth + realFinDay;
										_requirement.setRealFinYmd(realFinYmd);
									}
								}
								break;
							case 39 :
								...
								//-----------------------------------------------------------------
								{
									...
									if( Util.isEmpty(cols[j]) ) {
										...
										if(cols[j].trim().matches(".*2.*")) {
											...
											throw new Exception("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm + "] 값이 올바르지 않습니다.(구분자 2로 구분하여 주십시오.)");
										}
									}
								}							
								break;
								...
								//-----------------------------------------------------------------
								{
									...
									if( Util.isEmpty(cols[j]) ) {
										...
										if(cols[j].trim().matches(".*,.*")) {
											...
											throw new Exception("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm + "] 값이 올바르지 않습니다.(구분자 ,로 구분하여 주십시오.)");
										}
										...
										if(cols[j].trim().matches(".*,.*")) {
											...
											if(isSeqGroup) {
												for(int k = 0; k < dataAccessRoles.length; k++) {
													boolean seqGroupMatchFlag = false;
													for(DataAccessRole dar : _dataAccessRoleList) {
														if(dar.getRoleCd().equals(dataAccessRoles[k].trim())) {
															seqGroupMatchFlag = true;
															break;
														}
													}
													if(!seqGroupMatchFlag) {
												...
												for(DataAccessRole dar : _dataAccessRoleList) {
													...
													//DataAccessRole defaultDataAccessRole = null;
													DataAccessRole matchDataAccessRole = null;
													
													for(int k=0; k < dataAccessRoles.length; k++) {
														boolean matchFlag = false;
														
														// KICS일 경우 아래의 로직을 skip하고 for문으로 간다.
														if(dataAccessRoles[k].trim().equals("KICS")) {
															matchFlag = true;
															continue;
														}
														...
														if(matchFlag) {
															...
															throw new Exception("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm + "] 값 " + dataAccessRoles[k] + "는 등록되어있지 않은 값입니다.");
															//break;
														}
													}
													...
													// KICS기본셋팅
													for(DataAccessRole dar : ckeckDataAccessRoleList) {
														if(dar.getRoleCd().equals("KICS")) {
															defaultDataAccessRole = dar;
															break;
														}
													}
													_dataAccessRoleList.add(defaultDataAccessRole);
