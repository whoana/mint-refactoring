...
public class HdinsExcelUploadService implements IExcelUploadService {
...
private static final Logger logger = LoggerFactory.getLogger(ExcelUploadService.class);
	...
	public List<Requirement> validate(List<String[]> list, Map<String,Object> params) throws Exception {
		...
		//----------------------------------------------------------------------------
				...
				// 인터페이스 유형에 대한 정보를 담는다. 2017-08-07 현대해상 맵핑
				List<CommonCode> checkHasmapData = commonService.getCommonCodeList("AN", "14");
				...
				for( int i = 0; i < list.size(); i++ ) {
					...
					if( isSeqGroup ) {
						...
						_interfaceProperty  = _interfacez.getProperties();
						...
						_interfaceProperty  = new LinkedList<InterfaceProperties>();
						//-----------------------------------------------------------------
						...
						_interfacez.setProperties(_interfaceProperty);
					...
					for( int j = 0; j < cols.length; j++ ) {
						switch( j ) {
							...
							case 23 :
								//-----------------------------------------------------------------
								// ★ 시스템-송신시스템대분류코드 --> 현대해상은 중분류코드를 화면에 보이게
								//-----------------------------------------------------------------
								/* if( isSeqGroup ) break;
								{
									String colNm = "시스템-송신시스템대분류코드";
									if( Util.isEmpty(cols[j]) ) {
										throw new NullPointerException("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm + "] 값이 공백입니다.");
									} else {
										String cd = "";
							...
							case 29 :
								//-----------------------------------------------------------------
								// ☆ 시스템-수신시스템대분류코드 -> 현대해상은 증분류코드가 화면에 보이게
								//-----------------------------------------------------------------
								/* {
									String colNm = "시스템-수신시스템대분류코드";
									if( Util.isEmpty(cols[j]) ) {
										//if( isSeqGroup ) break;
										throw new NullPointerException("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm + "] 값이 공백입니다.");
									} else {
							...
							case 39 :
								...
								if( isSeqGroup ) break;
								{
									...
									if( Util.isEmpty(cols[j]) ) {
										...
										//-----------------------------------------------------------------
										// 전사솔루션 담당자 추가(형통은 사용안하기떄문에 일단 주석처리)
										//-----------------------------------------------------------------
										/*{
							...
							case 41 :
								//-----------------------------------------------------------------
								// ☆ 연계기본정보-인터페이스구분 (온라인, 배치)
								//-----------------------------------------------------------------
								if( isSeqGroup ) break;
								{
									String colNm = "연계기본정보-인터페이스구분";
									if( Util.isEmpty(cols[j]) ) {
										throw new NullPointerException("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm + "] 값이 공백입니다.");
									} else {
										...
										boolean matchFlag = false;
										for(CommonCode commonCode : checkHasmapData) {
											if(commonCode.getNm().equals(cols[j].trim())) {
												cd = commonCode.getCd();
												nm = commonCode.getNm();
												matchFlag = true;
										...
										if( matchFlag ) {
											_interfacez.setHasDataMap(cd);
											_interfacez.setHasDataMapNm(nm);
										} else {
											throw new Exception("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm + "] 값이 올바르지 않습니다.");
										}
									}
							...
							case 43 :
								...
								if( isSeqGroup ) break;
								{
									String colNm = "스케줄-발생유형";
									if( Util.isEmpty(cols[j]) ) {
										throw new NullPointerException("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm + "] 값이 공백입니다.");
									} else {
										// 아래의 동기, 비동기 코드는 어떻게 형통에서 적용할 것인지 판단하여. DB의 값을 동기 -> Online, 비동기 -> Batch로 바꾸던지
										// 혹은 현재 상태로 두던지 판별필요.
										/*String appPrMethod = "";
										if(cols[j].trim().equals("Online")) {
											appPrMethod = "동기";
										} else if(cols[j].trim().equals("Batch")) {
											appPrMethod = "비동기";
										} else {
											throw new Exception("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm + "] 값이 올바르지 않습니다.");
										}*/
										...
										for(CommonCode commonCode : checkAppPrMethod) {
											if(commonCode.getNm().equals(cols[j].trim())) {
											//if(commonCode.getNm().equals(appPrMethod)) {
												cd = commonCode.getCd();
												nm = commonCode.getNm();
												matchFlag = true;
												break;
							...
							case 48 :
								//-----------------------------------------------------------------
								// ☆ 관리정보-연계패턴(형통은 DB2DB, APP2DB 등 2를 기준인 형식으로 되어있음.)
								//-----------------------------------------------------------------
								{
									String colNm = "관리정보-연계패턴";
									if( Util.isEmpty(cols[j]) ) {
										throw new NullPointerException("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm + "] 값이 공백입니다.");
									} else {
										if(cols[j].trim().matches(".*2.*")) {
											String[] systemResources = cols[j].trim().split("2");

											if(systemResources.length >= 2) {
												...
												for(int k = 0; k < 2; k++) {
													for(CommonCode commonCode : checkResourceCd) {
														if(commonCode.getNm().equalsIgnoreCase(systemResources[k].trim())) {
															if(k == 0) {
												...
												if( consumerMatchFlag && providerMatchFlag ) {
													if( !isSeqGroup ) {
									...
									for(Channel channel : checkChannelList){
										for(Map chalAttr :  channel.getRelChlAttributes()){
											...
											if(((String)chalAttr.get("attrNm")).equalsIgnoreCase("수신목적지_개발")) {
												...
												logger.debug(""+interfaceProp.toString());
												_interfaceProperty.add(interfaceProp);
								...
								//-----------------------------------------------------------------
							{
									for(Channel channel : checkChannelList){
										for(Map chalAttr :  channel.getRelChlAttributes()){
											...
											if(((String)chalAttr.get("attrNm")).equalsIgnoreCase("수신목적지_검증")) {
												InterfaceProperties interfaceProp = new InterfaceProperties();
												interfaceProp.setAttrId((String)chalAttr.get("attrId"));
												interfaceProp.setAttrValue(cols[j].trim());
								...
								//-----------------------------------------------------------------
							{
									for(Channel channel : checkChannelList){
										for(Map chalAttr :  channel.getRelChlAttributes()){
											...
											if(((String)chalAttr.get("attrNm")).equalsIgnoreCase("수신클래스")) {
												InterfaceProperties interfaceProp = new InterfaceProperties();
												interfaceProp.setAttrId((String)chalAttr.get("attrId"));
												interfaceProp.setAttrValue(cols[j].trim());
								...
								//-----------------------------------------------------------------
							{
									for(Channel channel : checkChannelList){
										for(Map chalAttr :  channel.getRelChlAttributes()){
											...
											if(((String)chalAttr.get("attrNm")).equalsIgnoreCase("수신메소드")) {
												InterfaceProperties interfaceProp = new InterfaceProperties();
												interfaceProp.setAttrId((String)chalAttr.get("attrId"));
												interfaceProp.setAttrValue(cols[j].trim());
								...
								//-----------------------------------------------------------------
							{
									for(Channel channel : checkChannelList){
										for(Map chalAttr :  channel.getRelChlAttributes()){
											...
											if(((String)chalAttr.get("attrNm")).equalsIgnoreCase("수신목적지_운영")) {
												InterfaceProperties interfaceProp = new InterfaceProperties();
												interfaceProp.setAttrId((String)chalAttr.get("attrId"));
												interfaceProp.setAttrValue(cols[j].trim());
								...
								//-----------------------------------------------------------------
							{
									for(Channel channel : checkChannelList){
										for(Map chalAttr :  channel.getRelChlAttributes()){
											...
											if(((String)chalAttr.get("attrNm")).equalsIgnoreCase("수신서비스")) {
												InterfaceProperties interfaceProp = new InterfaceProperties();
												interfaceProp.setAttrId((String)chalAttr.get("attrId"));
												interfaceProp.setAttrValue(cols[j].trim());
								...
								//-----------------------------------------------------------------
							{
									for(Channel channel : checkChannelList){
										for(Map chalAttr :  channel.getRelChlAttributes()){
											...
											if(((String)chalAttr.get("attrNm")).equalsIgnoreCase("인터페이스구분")) {
												InterfaceProperties interfaceProp = new InterfaceProperties();
												interfaceProp.setAttrId((String)chalAttr.get("attrId"));
												interfaceProp.setAttrValue(cols[j].trim());
	...
	@Override
	public List<Requirement> validate(List<String[]> list,
			List<String[]> headerlist, Map<String, Object> params)
			throws Exception {
