...
public class GSSPExcelUploadService implements IExcelUploadService {
	...
	public List<Requirement> validate(List<String[]> list, List<String[]> headerlist, Map<String,Object> params) throws Exception {
		...
		List<pep.per.mint.common.data.basic.System> checkSystemCdList = commonService.getRootSystemCdList();
		// TSU0301 테이블(공통코드테이블)
			...
			for( int j = 0; j < cols.length; j++ ) {
					...
					//-----------------------------------------------------------------
				{
					...
					if( Util.isEmpty(cols[j]) ) {
						...
						String[] systemCds = cols[j].trim().split(",");
						
						if(systemCds.length>0) {
							
							HashSet<String> systemCdsSet = new HashSet<String>();
							
							for(int k=0; k<systemCds.length; k++) {
								systemCdsSet.add(systemCds[k].trim());
							}
							
							Iterator<String> iterator = systemCdsSet.iterator();
							while(iterator.hasNext()) {
								String systemCd = iterator.next().trim();
								pep.per.mint.common.data.basic.System matchSystem = null;
								boolean matchFlag = false;
								for(pep.per.mint.common.data.basic.System system : checkSystemCdList) {
									if(system.getSystemCd().equalsIgnoreCase(systemCd)) {
										matchSystem = new pep.per.mint.common.data.basic.System();
										matchSystem.setSystemCd(system.getSystemCd());
										matchSystem.setSystemId(system.getSystemId());
										matchSystem.setSystemNm(system.getSystemNm());
										matchFlag = true;
										break;
									}
								}
								if( matchFlag ) {
									matchSystem.setSeq(_systemList.size());
									matchSystem.setNodeType("2");
									matchSystem.setNodeTypeNm("수신");
									matchSystem.setDelYn("N");
									matchSystem.setRegId(params.get("userId").toString());
									matchSystem.setModId(params.get("userId").toString());
									matchSystem.setRegDate(params.get("regDate").toString());
									matchSystem.setModDate(params.get("regDate").toString());
									_systemList.add(matchSystem);
								} else {
									throw new Exception("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm + "] 값이 올바르지 않습니다.");
								}
							}
				...
				case 39 :
					...
					//-----------------------------------------------------------------
				{
					String colNm = "연계기본정보-진행상태";
					if( Util.isEmpty(cols[j]) ) {
						throw new NullPointerException("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm + "] 값이 공백입니다.");
					} else {
						if(cols[j].equalsIgnoreCase("수정")){
							_interfacez.setStatus("1");
						
							
						}else{
							_interfacez.setStatus("0");					
						}
					}
				}
				break;
				...
				case 44 :
					...
					if( isSeqGroup ) break;
					{
						...
						if( providerMatchFlag ) {
							for(int k=0; k<_systemList.size(); k++) {
								if(_systemList.get(k).getNodeType().equals("2")) {
									_systemList.get(k).setResourceCd(providerResourceCd);
									_systemList.get(k).setResourceNm(providerResourceNm);
								}
							}
						} else {
							throw new Exception("[ 엑셀 업로드 실패 ] : " + (i+1) +" 번째 행, [" + colNm+ "] 값이 올바르지 않습니다.");
				...
				{
					for(int k=0; k<_systemList.size(); k++) {
						pep.per.mint.common.data.basic.System system =_systemList.get(k);
						if(system.getNodeType().equals("2")) {
							system.setService(cols[j].trim());
						}
					...
					//-----------------------------------------------------------------
				{
					...
					for(int k=0; k<_systemList.size(); k++) {
						if(_systemList.get(k).getNodeType().equals("2")) {
							_systemList.get(k).setServiceDesc(cols[j].trim());
						}
					}
				}
				break;
	...
	public int createRequirement(List<Requirement> requirementList, Map<String,Object> params) throws Exception {
		...
		try{
			...
			for(Requirement requirement : requirementList) {
				...
				// 일괄 업로드시 interface.status 값이 1(수정)일 때 업데이트 처리하는 로직 추가 2018.08.31
				if( requirement.getInterfaceInfo().getStatus().equalsIgnoreCase("1")  ) {							
					Map<String, Object> updateRequirementMap=requirementService.getRequirementIdForUpdate(requirement.getInterfaceInfo().getIntegrationId());
					requirement.setRequirementId(""+updateRequirementMap.get("REQUIREMENTID"));					
					
					Interface updateInterfaceObj =requirement.getInterfaceInfo();
					updateInterfaceObj.setInterfaceId(""+updateRequirementMap.get("INTERFACEID"));
					requirement.setInterfaceInfo(updateInterfaceObj);					
					
					//Excel 업로드 기능 update를 위해서 ,true 입력
					resultCd = requirementService.updateRequirement(requirement, true);
