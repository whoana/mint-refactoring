...
public class SSHPExcelUploadService implements IExcelUploadService {
							...
							case 42 :
								...
								if( isSeqGroup ) break;
								{
									...
									if( Util.isEmpty(cols[j]) ) {
										...
										// 혹은 현재 상태로 두던지 판별필요.
										String appPrMethod = cols[j].trim();
								...
								if( isSeqGroup ) break;
								{
									...
									if( Util.isEmpty(cols[j]) ) {
										...
										try {
											...
											User checkUser = null;
											try {
												checkUser = commonService.getUser(cols[j]);
											} catch(Exception e) {
											}

											if( checkUser != null && !Util.isEmpty(checkUser.getUserId()) ) {
												_requirement.setRegId(cols[j]);
												_requirement.setModId(cols[j]);

												_interfacez.setRegId(cols[j]);
												_interfacez.setModId(cols[j]);
											}
										} catch( Exception e ) {
											_requirement.setRegId(params.get("userId").toString());
											_requirement.setModId(params.get("userId").toString());
